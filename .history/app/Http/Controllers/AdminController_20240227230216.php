...
class AdminController extends Controller
{
    ...
    public function getDataAdmin()
    {
        $data = Admin::join('quyens', 'admins.id_permission', 'quyens.id')
                     ->select('admins.id', 'admins.first_last_name', 'admins.email', 'admins.phone_number', 'admins.status', 'admins.id_permission', DB::raw("DATE_FORMAT(admins.date_birth, '%d-%m-%Y') as date_birth"), 'quyens.ten_quyen')
                     ->paginate(10);
        $response = [
            'pagination' => [
                'total' => $data->total(),
